...
import io.sseg.boundedcontext.user.constants.AccountRegistrationPolicy;
...
public class AccountDtoValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if(accountRepository.existsByUsername(account.getUsername())) {
            errors.rejectValue("username", "username", AccountRegistrationPolicy.ViolationType.USERNAME_DUPLICATION);
        }
        
        // 사용할 수 없는 username
        if(AccountRegistrationPolicy.Constraints.getInvalidUsernames().contains(account.getUsername())) {
            errors.rejectValue("username", "username", AccountRegistrationPolicy.ViolationType.INVALID_USERNAME);
        }
        
        
        // email 중복검사 (provider 별로..)
        if(accountRepository.existsByEmailAndProvider(account.getEmail(), account.getProvider())) {
            errors.rejectValue("email", "email", AccountRegistrationPolicy.ViolationType.EMAIL_DUPLICATION);
        }
        ...
        if(account instanceof VerifyRequestRegisterForm verifyRequestRegisterForm) {
            if(!verifyRequestRegisterForm.getPassword().equals(verifyRequestRegisterForm.getPasswordConfirm())) {
                errors.rejectValue("passwordConfirm", "passwordConfirm", AccountRegistrationPolicy.ViolationType.INVALID_PASSWORD_CONFIRM);
            }
        }
