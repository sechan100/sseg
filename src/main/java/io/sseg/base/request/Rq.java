...
public class Rq {
    ...
    public <T extends UserOwnable> void isAccessAllowed(Long id, Class<T> clazz) {
        ...
        if(repository == null) {
            throw new AccessDeniedException("소유권 확인 대상 엔티티의 레포지토리가 존재하지 않습니다. repository 빈이 존재하지 않거나, repository 빈의 이름이 네이밍 컨벤션에 부합하지 않을 수 있습니다. (네이밍 컨벤션: 엔티티 이름의 첫 글자를 소문자로 바꾸고, 엔티티 이름 + Repository");
        }
        ...
        if (targetEntity.getOwner() != null) {
            ...
            boolean isUserOwner = Objects.equals(targetEntity.getOwner().getId(), this.account.getId());
